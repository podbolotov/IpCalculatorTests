...
"$PROJECT_ROOT"/allure-report/"$CURRENT_TIME"/multi-file --name "IP Calculator Tests"
echo "[ 🤖 Generating single-file report with Allure... ]"
allure generate "$PROJECT_ROOT"/allure-results --clean --output \
"$PROJECT_ROOT"/allure-report/"$CURRENT_TIME"/single-file --single-file --name "IP Calculator Tests"
